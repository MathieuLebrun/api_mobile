...
async function getAIResponse(userMessage, history) {
  ...
  if (history.length === 0) {
    ...
    history.push({ sender: 'Vous', text: userMessage });
  } else {
    history.push({ sender: 'Vous', text: userMessage });
  }
  ...
  try {
    ...
    history.push({ sender: 'model', text: text });
...
router.get('/helio/historique/:id_helio', async (req, res) => {
    const { id_helio } = req.params;
    try {
        const existingMessage = await Helio.findByPk(id_helio);
        if (!existingMessage) {
            return res.status(404).json({ response: 'Conversation non trouvée.' });
        }
        let history;
        try {
            history = JSON.parse(existingMessage.historique || '[]');
        } catch (error) {
            console.error('Erreur de parsing JSON:', error);
            return res.status(500).json({ response: 'Erreur de parsing JSON.' });
        }
        console.log(history);
        res.json({ response: JSON.stringify(history)});
...
router.post('/helio/chat/:id_helio', async (req, res) => {
    ...
    try {
        ...
        existingMessage.historique = JSON.stringify(aiResponse.history);
...
router.post('/helio/chat', async (req, res) => {
    const { message, id_projet } = req.body;
    var history = [];
    ...
    try {
        ...
        const newMessage = await Helio.create({
            titre: 'Conversation avec Hélio',
            statut_du_module: 'En cours',
            historique: JSON.stringify(aiResponse.history),
